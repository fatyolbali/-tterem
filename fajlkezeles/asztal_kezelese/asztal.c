...
AsztalElem *asztal_eldonto(AsztalElem *lista)
{
    ...
    {
        int id_buffer, ferohely_buffer, sor, oszlop, foglalt;

        printf("Asztalok létrehozása (0 férőhely a befejezéshez):\n");
        while (true)
        {
            printf("Asztal sorszáma, férőhelyek száma, sor, oszlop: ");
            if (scanf("%d %d %d %d", &id_buffer, &ferohely_buffer, &sor, &oszlop, &foglalt) != 4)
            {
                break;
            }
            ...
            lista = asztal_letrehoz(lista, id_buffer, ferohely_buffer, sor, oszlop, foglalt);
        }
    }
